...
class True_Transformer(nn.Module):
    def __init__(self, n_heads, int_dim):
        ...
        self.int_dim     = int_dim
        self.n_heads     = n_heads
        self.linear      = nn.Linear(int_dim,int_dim)
        self.act         = nn.Tanh()
        self.norm        = torch.nn.BatchNorm1d(int_dim*n_heads)

    def forward(self,x):
        batchsize = x.shape[0]
        out = torch.reshape(self.norm(x),(batchsize,self.n_heads,self.int_dim))
        out = self.act(self.linear(out))
        out = torch.reshape(out,(batchsize,self.n_heads*self.int_dim))
        return out+x
...
class nn_pca_emulator:
    def __init__(self, 
        ...
        if self.optim is None:
            ...
            self.optim = torch.optim.Adam(self.model.parameters(), lr=lr ,weight_decay=1e-3)
    ...
    def load(self, filename, device=torch.device('cpu')):
        ...
        try:
            ...
            print('Failed to load using "torch.load(file)", trying again using "torch.load_state_dict(torch.load(file))"...')
            self.model.load_state_dict(torch.load('./projects/lsst_y1/emulator_output/god_model_dict',map_location=device))
        summary(self.model)
        self.model.eval()
        ...
        with h5.File(filename + '.h5', 'r') as f:
            ...
            #self.evecs_cpu = self.evecs.cpu().detach().numpy()
            #self.dv_fid_cpu = self.dv_fid.cpu().detach().numpy()
